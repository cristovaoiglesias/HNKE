...
for ensemble_size=2:100
   ...
   subMLPidx = sample(1:100, ensemble_size, replace = false)
    ...
    for e in xdt[dt_idx:dt_idx]#[9:10]
        #e = xdt[10:10][1]
        xtest = e[1]
        ...
        # 6.023377236515776, 0.6190039484388988 ,4.29859924611443, 0.7052432546582847,141.9823209918635
     ...
     # #### HM+CKF
            ...
            append!(NIS_ENN_MM_CKF,ie*inv(S)*ie)

            e_x=[sol_gt[k+1,:]-m]
            append!(NEES_ENN_MM_CKF,[[   e_x[1][1]* inv(P[1,1])* e_x[1][1],
            ...
            e_x=sol_gt[k+1,:]-m
            append!(NEES_ENN_MM_CKF_total, e_x'* inv(P)* e_x)
     ...
     # #### Classic CKF
            ...
            e_x=[sol_gt[k+1,:]-m2]
            append!(NEES_ckf,[[  e_x[1][1]* inv(P2[1,1])* e_x[1][1],
            ...
            e_x=sol_gt[k+1,:]-m2
            append!(NEES_ckf_total, e_x'* inv(P2)* e_x)
        ...
        plot!(tgrid_opt[2:end], MM', label = "HNKE-C", grid=false,fillalpha=.3,color=:blue, lw=lws,layout=(3,2))
        display(plots)
        savefig("HNKE-C_$(ensemble_size)")
        ...
        CSV.write(str_title*"CKF.csv",chain_df)


        println("-----RMSPE HNKE-C------ensemble size ", ensemble_size," - ",subMLPidx)
        ...
        append!(RMSPE_HNKEc_EnSizes,  [[RMSPE(sol_gt[2:end,1],MM'[:,1]),
...
RMSPE_HNKEc_EnSizes=vcat(map(x->x', RMSPE_HNKEc_EnSizes)...)
chain_df = DataFrame(RMSPE_HNKEc_EnSizes, :auto)
CSV.write("RMSPE_HNKEc_EnSizes_$(xdt[dt_idx:dt_idx][1][3]).csv",chain_df)

  #plots
lws=2.5
gr( xtickfontsize=7, ytickfontsize=7, xguidefontsize=9, yguidefontsize=9, legendfontsize=6);
plots=plot(Array(2:1:100),RMSPE_HNKEc_EnSizes,color=:green , lw=lws, label = "RMSPE HNKE-C", xlabel="Ensemble size", ylabel=["Xv(Cell/L)" "GLC(mM)" "GLN(mM)" "LAC(mM)" "AMM(mM)" "mAb(mg/L)"], layout=(3,2),size = (800,700))
display(plots)
savefig("RMSPE_HNKEc_EnSizes_$(xdt[dt_idx:dt_idx][1][3])")
